...
// Test poprawnego uruchomienia worker
void test_worker_startup_valid() {
    pid_t pid = fork();
    if (pid == 0) {
        // Proces dziecka: uruchomienie worker
        execl("./worker", "./worker", "1", NULL);
        perror("Nie udalo sie uruchomic worker");
        exit(EXIT_FAILURE);
    } else if (pid > 0) {
        // Proces rodzica: sprawdzenie inicjalizacji
        sleep(3); // Czekanie na inicjalizacje worker
        int shmId = shmget(SHM_KEY, sizeof(struct Memory), 0666);
        assert(shmId != -1);

        struct Memory *sharedMemory = (struct Memory *)shmat(shmId, NULL, 0);
        assert(sharedMemory != (void *)-1);
        assert(sharedMemory->pid_worker1 != 0); // Sprawdzanie PID worker 1

        kill(sharedMemory->pid_worker1, SIGTERM); // Zatrzymanie worker
        shmdt(sharedMemory);
        wait(NULL);        // Czekanie na zakonczenie procesu dziecka
        printf("Test zaliczony: Poprawne uruchomienie worker 1\n");
